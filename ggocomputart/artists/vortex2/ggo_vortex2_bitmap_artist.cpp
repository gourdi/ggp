...
void ggo_vortex2_bitmap_artist::render_bitmap(uint8_t * buffer)
{
	...
	params._vortices.set_size(GGO_VORTEX2_VORTICES_COUNT);
	...
	for (int i = 0; i < params._vortices.get_size(); ++i)
	{
		params._vortices[i]._pos.x() = ggo::rand_float() * get_render_width();
		params._vortices[i]._pos.y() = ggo::rand_float() * get_render_height();
		params._vortices[i]._angle = ggo::rand_float(ggo::PI<float>() / 2, ggo::PI<float>() / 2 + 0.5f);
		params._vortices[i]._speed = ggo::rand_float(0.004f, 0.008f) * get_render_max_size();
		params._vortices[i]._power = ggo::rand_float(6.f, 8.f);
	}

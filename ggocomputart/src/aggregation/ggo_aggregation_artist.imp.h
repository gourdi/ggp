...
void ggo::aggregation_artist::render(void * buffer, int line_step) const
{
  ...
  ggo::fill_solid<format>(buffer, get_width(), get_height(), line_step, ggo::convert_color_to<color_t>(_background_color), ggo::rect_int::from_width_height(get_width(), get_height()));

  {
    std::vector<ggo::solid_color_shape<ggo::disc_float, color_t>> shapes;

    for (const auto & cell : _grid)
    {
      for (const auto & point : cell._points)
      {
        shapes.emplace_back(ggo::disc_float(point._pos, 2.f * _threshold_dist), ggo::black<color_t>());
      }
    }
...

  {
    ...
    for (const auto & cell : _grid)
    {
      for (const auto & point : cell._points)
      {
        ...
        color_t c = ggo::convert_color_to<color_t>(ggo::from_hsv<ggo::color_8u>(hue, sat, point._val));

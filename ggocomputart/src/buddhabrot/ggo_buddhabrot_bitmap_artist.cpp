#include "ggo_buddhabrot_bitmap_artist.h"
...
{
  //////////////////////////////////////////////////////////////
  bool iterate(const std::complex<double> & p, std::vector<std::complex<double> > & points, int escape_threshold)
  {
    points.clear();
  ...
  //////////////////////////////////////////////////////////////
  bool iterate(const std::complex<double> & p, int escape_threshold)
  {
  ...
  //////////////////////////////////////////////////////////////
  bool discard_point(const std::complex<double> & p)
  {
    double x = p.real();
    double y = p.real();
  ...
  //////////////////////////////////////////////////////////////
  void setup_seed_area(int escape_threshold_r, int escape_threshold_g, int escape_threshold_b, std::complex<double> & center, double & range)
  {
    int counter_max = 0;
    for (int j = 0; j < 100; ++j)
    {
  ...
  //////////////////////////////////////////////////////////////
  class buddhabrot_artist : public artist
  {
  public:

    //////////////////////////////////////////////////////////////
    buddhabrot_artist(int width, int height) : artist(width, height) {}
    ...
    void render_bitmap(void * buffer, int line_byte_step) const
    {
      ...
      for (int y = 0; y < height(); ++y)
      {
        ...
        {
          uint8_t r = std::min(accumulation_r(x, y), 255);
          uint8_t g = std::min(accumulation_g(x, y), 255);
          uint8_t b = std::min(accumulation_b(x, y), 255);
...
//////////////////////////////////////////////////////////////
void ggo::buddhabrot_bitmap_artist::render_bitmap(void * buffer, int width, int height, int line_byte_step, ggo::pixel_type pixel_type, ggo::lines_order memory_lines_order) const
{
  buddhabrot_artist artist(width, height);

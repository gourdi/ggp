...
GGO_TEST(collisions, half_plane_vs_oriented_box)
{
  ...
  GGO_CHECK_FABS(pos.get<0>(), 3.0f);
  GGO_CHECK_FABS(pos.get<1>(), 2.0f);
  GGO_CHECK_FABS(normal.get<0>(), 0.0f);
  GGO_CHECK_FABS(normal.get<1>(), 1.0f);
}
...
GGO_TEST(collisions, half_plane_vs_oriented_box_horizontal)
{
  ...
  ggo::vec2f normal;
  GGO_CHECK(ggo::test_collision(half_plane, box, pos, normal) == true);
  GGO_CHECK_FABS(pos.get<0>(), 4.0f);
  GGO_CHECK_FABS(pos.get<1>(), 3.0f);
  GGO_CHECK_FABS(normal.get<0>(), 0.0f);
  GGO_CHECK_FABS(normal.get<1>(), 1.0f);
}
...
GGO_TEST(collisions, oriented_box_vs_oriented_box_one_point_inside)
{
  ...
  // Intersection on top edge of box1.
  {
    ...
    GGO_CHECK_FABS(pos.get<0>(), 6.0f);
    GGO_CHECK_FABS(pos.get<1>(), 5.0f);
    GGO_CHECK_FABS(normal.get<0>(), 0.0f);
    GGO_CHECK_FABS(normal.get<1>(), 1.0f);
  }
  ...
  // Intersection on left edge of box1.
  {
    ...
    ggo::vec2f normal;
    GGO_CHECK(ggo::test_collision(box1, box2, pos, normal) == true);
    GGO_CHECK_FABS(pos.get<0>(), 2.0f);
    GGO_CHECK_FABS(pos.get<1>(), 3.0f);
    GGO_CHECK_FABS(normal.get<0>(), -1.0f);
    GGO_CHECK_FABS(normal.get<1>(), 0.0f);
  }
}

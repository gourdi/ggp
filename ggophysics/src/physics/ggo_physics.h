...
namespace ggo
{
  ...
  struct rigid_body
  {
    ...
    virtual impulse     compute_impulse(const ggo::pos2f & pos, const ggo::vec2f & normal) const = 0;
    virtual ggo::pos2f  get_center_of_mass() const = 0;

    void apply_impulse(float impulse_intensity, const ggo::pos2f & pos, const ggo::vec2f & normal)
    {
  ...
  {
    half_plane_body(const ggo::vec2f & normal, float dist_to_origin) : _half_plane(normal, dist_to_origin) {}

    impulse compute_impulse(const ggo::pos2f & pos, const ggo::vec2f & normal) const override
    {
      return { {0.f, 0.f}, 0.f };
    }

    ggo::pos2f  get_center_of_mass() const override
    {
      return _half_plane.normal() * _half_plane.dist_to_origin();
    }

    ggo::half_plane_float _half_plane;
  };
  ...
  struct oriented_box_body : public rigid_body
  {
    ...
    impulse compute_impulse(const ggo::pos2f & pos, const ggo::vec2f & normal) const
    {
      impulse impulse;

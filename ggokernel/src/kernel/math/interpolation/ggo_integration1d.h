...
{
  //////////////////////////////////////////////////////////////
  template <typename data_t, typename in_t, typename make_polynom_t, typename scalar_t>
  data_t integration1d(in_t && in, make_polynom_t && make_polynom, scalar_t from, scalar_t to)
  {
    data_t val(0);
...
namespace ggo
{
  ...
  data_t linear_integration1d(in_t && in, scalar_t from, scalar_t to)
  {
    ...
    return integration1d<data_t>(in, make_polynom, from, to);
  }
  ...
  {
    return linear_integration1d<data_t>([&](int i) { return in[i]; }, from, to);
  }
}
...
namespace ggo
{
  //////////////////////////////////////////////////////////////
  template <typename data_t, typename in_t, typename scalar_t>
  data_t cubic_integration1d(in_t && in, scalar_t from, scalar_t to)
  {
    auto make_polynom = [&](int lower_sample_index)
    {
  ...
  {
    return cubic_integration1d<data_t>([&](int i) { return in[ggo::clamp(i, 0, size - 1)]; }, from, to);
  }
}

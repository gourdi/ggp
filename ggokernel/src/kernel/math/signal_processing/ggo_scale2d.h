...
namespace ggo
{
  ...
  void resample(in_t && in, int width_in, int height_in, out_t && out, int width_out, int height_out)
  {
    ...
    const scalar_t horz_ratio = scalar_t(width_in - 1) / scalar_t(width_out - 1);
    const scalar_t vert_ratio = scalar_t(height_in - 1) / scalar_t(height_out - 1);
    ...
    auto process = [&](ggo::rect_int tile)
    {
      ...
      {
        data_t v = ggo::interpolation2d<interp, data_t>(in_clampped, x * horz_ratio, y * vert_ratio);
...
//{
//  //////////////////////////////////////////////////////////////
//  template <typename scanner2d_t, typename tiling_y, typename scalar_t, typename in_t, typename out_t>
//  void scale_2d(in_t && in, int width_in, int height_in, out_t && out, int width_out, int height_out, scaling_algo algo)
//  {
...
//      switch (algo)
//      {
//      case scaling_algo::linear_resampling:
//        resample<ggo::interpolation2d_type::bilinear, scanner2d_t, tiling_y, scalar_t>(in, width_in, height_in, out, width_out, height_out);
//        break;
//      case scaling_algo::cubic_resampling:
//        resample<ggo::interpolation2d_type::bicublic, scanner2d_t, tiling_y, scalar_t>(in, width_in, height_in, out, width_out, height_out);
//        break;
//      }
//    }
//  }

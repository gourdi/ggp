...
#define GGO_CHECK_EQ(v1, v2) { auto eval1 = (v1); auto eval2 = (v2); if (eval1 != eval2) { ++_failed_count; ggo::color_stream cs(ggo::console_color::RED); cs << "** TEST FAILED ** " << __FILE__ << ':' << __LINE__ << ": " << #v1 << " [=" << eval1 << "] != " << #v2 << " [=" << eval2 << "]" << std::endl; } }

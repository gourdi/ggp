...
GGO_TEST(basis2d, local_world)
{
  ...
  ggo::pos2_f p1 = basis.point_from_local_to_world({ 1.f, 2.f });
  GGO_CHECK_FLOAT_EQ(p1.x(), 3.f);
  GGO_CHECK_FLOAT_EQ(p1.y(), 6.f);

  ggo::pos2_f p2 = basis.point_from_world_to_local({ 3.f, 6.f });
...
GGO_TEST(orthogonal_basis2d, local_world)
{
  ...
  ggo::pos2_f p2 = basis.point_from_world_to_local({ 7.f, 6.f });
...
GGO_TEST(orthonormal_basis2d, local_world)
{
  ...
  ggo::pos2_f p1 = basis.point_from_local_to_world({ 3.f, 1.f });
  GGO_CHECK_FLOAT_EQ(p1.x(), 1.f);
  GGO_CHECK_FLOAT_EQ(p1.y(), 4.f);

  ggo::pos2_f p2 = basis.point_from_world_to_local({ 1.f, 4.f });

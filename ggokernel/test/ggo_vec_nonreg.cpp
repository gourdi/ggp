...
GGO_TEST(vec, operators)
{
...

  {
    ...
    GGO_CHECK_FABS(0.5f, b.x());
    GGO_CHECK_FABS(1.0f, b.y());
    GGO_CHECK_FABS(1.5f, b.z());
  }
}
...
GGO_TEST(vec, length)
{
  ...
  GGO_CHECK_FABS(1.f / std::sqrt(5.f), v.x());
  GGO_CHECK_FABS(2.f / std::sqrt(5.f), v.y());

  auto v2 = v.get_normalized();
  GGO_CHECK(v2.is_normalized());
  GGO_CHECK_FABS(1.f / std::sqrt(5.f), v2.x());
  GGO_CHECK_FABS(2.f / std::sqrt(5.f), v2.y());
}
...
{
  {
    ...
    GGO_CHECK_FABS(0.f, v.x());
    GGO_CHECK_FABS(1.f, v.y());
  }

  {
    ...
    GGO_CHECK_FABS(3.f, v.x());
    GGO_CHECK_FABS(2.f, v.y());
  }
}
...
GGO_TEST(vec, move)
{
  ggo::vec<int, 2, ggo::geometry_t> v(1, 2);
  v.move(3, 4);
  GGO_CHECK_FABS(4, v.x());
  GGO_CHECK_FABS(6, v.y());
}

//////////////////////////////////////////////////////////////////
GGO_TEST(vec, ortho_dot)
{
  GGO_CHECK_FABS(ggo::ortho_dot(ggo::vec<float, 2, ggo::geometry_t>(1.f, 2.f), ggo::vec<float, 2, ggo::geometry_t>(3.f, 4.f)), 2.f);
  GGO_CHECK_FABS(ggo::ortho_dot(ggo::vec<float, 2, ggo::geometry_t>(2.f, 1.f), ggo::vec<float, 2, ggo::geometry_t>(5.f, 4.f)), -3.f);
}
...
GGO_TEST(vec, basis)
{
...

  {
    ...
    GGO_CHECK_FABS(0.f, basis.first.x());
    GGO_CHECK_FABS(1.f, basis.first.y());
    GGO_CHECK_FABS(0.f, basis.first.z());
    GGO_CHECK_FABS(-1.f, basis.second.x());
    GGO_CHECK_FABS(0.f, basis.second.y());
    GGO_CHECK_FABS(0.f, basis.second.z());
  }
}

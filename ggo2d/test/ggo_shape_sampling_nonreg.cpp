...
GGO_TEST(shape_sampling, disc_uniform_sampling)
{
  ...
  for (int i = 0; i < (1<<14); ++i)
  {
    ...
    uint8_t * ptr = buffer + 3 * (y * SIZE + x);
    ptr[0] = 0xFF;
    ptr[1] = 0xFF;
    ptr[2] = 0xFF;
  }
  
  ggo::save_bmp("test_disc_uniform_sampling.bmp", buffer, SIZE, SIZE);
}
...
GGO_TEST(shape_sampling, disc_poisson_sampling)
{
  ...
  for (const auto & sample : samples)
  {
    ...
    ggo::paint(buffer, SIZE, SIZE, disc, ggo::color::WHITE);
  }
  
  ggo::save_bmp("test_disc_poisson_sampling.bmp", buffer, SIZE, SIZE);
}
...
GGO_TEST(shape_sampling, hemisphere_uniform_sampling)
{
  ...
  for (int i = 0; i < (1<<20); ++i)
  {
    ...
    uint8_t * ptr = buffer + 3 * (y_i * SIZE + x_i);
  ...
  ggo::save_bmp("test_hemisphere_uniform_sampling.bmp", buffer, SIZE, SIZE);
}
...
GGO_TEST(shape_sampling, hemisphere_poisson_sampling)
{
  ...
  ggo::save_bmp("test_hemisphere_poisson_sampling.bmp", buffer, SIZE, SIZE);
}

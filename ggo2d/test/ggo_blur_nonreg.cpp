...
namespace
{
  ...
  void gaussian_blur2d_test(void * buffer, int width, int height, int line_step, const std::string & filename)
  {
    ...
    {
      ggo::gaussian_blur2d<ggo::rgb_8u_yu>(buffer, width, height, line_step, 5.f);
    }
...
GGO_TEST(blur, mean_box_rgb8u)
{
  ...
  ggo::mean_box_blur2d<ggo::rgb_8u_yu>(buffer.data(), width, height, line_step, 9);
  ...
  ggo::save_bmp("mean_box_blur_rgb_8u_yu.bmp", buffer.data(), ggo::rgb_8u_yu, width, height, line_step);
}

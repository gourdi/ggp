#ifndef __GGO_FILL_4_COLORS__
#define __GGO_FILL_4_COLORS__
...
namespace ggo
{
  ...
  void fill_4_colors(image_type & image,
    const typename image_type::color_t & c1,
    const typename image_type::color_t & c2,
    const typename image_type::color_t & c3,
    const typename image_type::color_t & c4,
    std::optional<ggo::rect_int> clipping = {})
  {
    ...
    const floating_point_t inv_w = 1 / static_cast<floating_point_t>(image.width());
    const floating_point_t inv_h = 1 / static_cast<floating_point_t>(image.height());
    ...
    {
      auto color5 = (static_cast<floating_point_t>(y) * c1_fp + static_cast<floating_point_t>(h - y) * c2_fp) * inv_h;
      auto color6 = (static_cast<floating_point_t>(y) * c3_fp + static_cast<floating_point_t>(h - y) * c4_fp) * inv_h;
      ...
      for (int x = clipping->left(); x <= clipping->right(); ++x)
      {
        ...
        image.write_pixel(x, y, ggo::convert_color_to<image_type::color_t>(color7));
      }
    }
  }

...
namespace ggo
{
  inline constexpr uint8_t additive_blend(uint8_t bkgd_color, uint8_t brush_color)
  {
    return uint8_t(std::min<uint32_t>(0xff, uint32_t(bkgd_color) + uint32_t(brush_color)));
  }

  inline constexpr ggo::rgb_8u additive_blend(const ggo::rgb_8u& bkgd_color, const ggo::rgb_8u& brush_color)
  {
    return {
  ...
  inline constexpr float additive_blend(float bkgd_color, float brush_color)
  {
    return std::min(1.f, bkgd_color + brush_color);
  }

  inline constexpr rgb_32f additive_blend(const rgb_32f& bkgd_color, const rgb_32f& brush_color)
  {
    return {
      additive_blend(bkgd_color.r(), brush_color.r()),
      additive_blend(bkgd_color.g(), brush_color.g()),
...
namespace ggo 
{
  static_assert(ggo::additive_blend(ggo::rgb_8u(0x00, 0x80, 0xff), ggo::rgb_8u(0xff, 0xff, 0xff)) == ggo::rgb_8u(0xff, 0xff, 0xff));
}

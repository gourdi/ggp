...
namespace ggo
{
  ...
  struct blur_samples<blur_samples_type::disc_12_samples>
  {
    ...
    static std::vector<ggo::pos2<data_t>> get_samples(const data_t r)
    {
      const data_t r3 = r / 3;
  ...
  struct blur_samples<blur_samples_type::disc_32_samples>
  {
    ...
    static std::vector<ggo::pos2<data_t>> get_samples(const data_t r)
    {
      const data_t r1_5 = r / 5;
      const data_t r3_5 = 3 * r / 5;
  ...
  struct blur_samples<blur_samples_type::disc_52_samples>
  {
    ...
    static std::vector<ggo::pos2<data_t>> get_samples(const data_t r)
    {
      const data_t r1_7 = r / 7;
      const data_t r3_7 = 3 * r / 7;
      const data_t r5_7 = 5 * r / 7;
  ...
  void paint_blur_shape(const shape_t & shape, int width, int height, typename shape_t::data_t blur_radius, paint_pixel_t paint_pixel)
  {
    ...
    {
      const data_t x_fp = static_cast<data_t>(x);
      const data_t y_fp = static_cast<data_t>(y);

      int count = 0;

      for (const auto & sample : disc_samples)
      {
        const data_t sample_x = x_fp + sample.x();
        const data_t sample_y = y_fp + sample.y();

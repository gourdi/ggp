...
namespace ggo
{
  ...
  // Solid color shape.
  template <typename shape_type, typename color_type, typename brush_color_type = color_type>
  struct solid_color_shape : public paint_shape_abc<typename shape_type::data_t, color_type, brush_color_type>
  {
    ...
    solid_color_shape(const shape_t & shape, const brush_color_t & color) : _shape(shape), _color(color) {}
    ...
    color_t blend(int x, int y, const color_t & bkgd_color, const brush_color_t & brush_color) const { return ggo::convert_color_to<color_t>(brush_color); }
  };
}

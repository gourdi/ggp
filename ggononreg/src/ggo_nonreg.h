...
#define GGO_CHECK(cond) if (!(cond)) { ++_failed_count; std::cout << "\033[31m** TEST FAILED ** " << '[' << __FILE__ << ':' << __LINE__ << "\033[0m]" << std::endl; }

#define GGO_CHECK_FABS(v1, v2) {auto eval1 = (v1); auto eval2 = (v2); \
                                if (std::fabs(eval1 - eval2) > 0.0001) { \
                                  ++_failed_count; \
                                  std::cout << "\033[31m** TEST FAILED ** (" << __FILE__ << ':' << __LINE__ << ") " << \
                                  eval1 << " != " << eval2 << "\033[0m" << std::endl; \
                                } }
